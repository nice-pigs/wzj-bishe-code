...
const char* deviceName = "ESP32座位监测器";
...
void setup() {
    ...
    Serial.println("\n=== ESP32 Seat Detection with Multi-Sensors ===");
    Serial.println("Version: 2.0 (Multi-Sensor Support)");
    ...
    switch (type) {
        case 0x01: // 传感器数据
            if (length >= 5) {
                ...
                Serial.printf("[DATA] T:%d°C H:%d%% L:%d\n", 
        ...
        case 0xFF: // 完整数据包 (所有传感器)
            if (length >= 15) {
                ...
                Serial.printf("[DATA] Full: T:%d H:%d L:%d P1:%d P2:%d D:%d PIR:%d Occ:%d Dur:%d\n", 
                    seatData.temperature, seatData.humidity, seatData.lightLevel,
                    seatData.pressure1, seatData.pressure2, seatData.distance,
                    seatData.pirState, seatData.occupied, seatData.duration);
            }
            break;
    }
}
