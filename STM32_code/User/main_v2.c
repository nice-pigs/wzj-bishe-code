...
/*=============================================================================
 * 座位占用检测系统 v2.0 - STM32主程序
 * 功能：多传感器融合检测座位是否有人，并通过WiFi上报状态
 * 硬件：STM32F103C8T6 + ESP32-S3 + 压力 + 超声波 + 雷达 + PIR + DHT11 + 光敏
 *============================================================================*/
...
void Update_Seat_State_MultiSensor(void);
...
int main(void)
{
    ...
    printf("[SYSTEM] Seat detection system v2.0 started!\r\n\r\n");
    ...
    while (1)
    {
        ...
        if(++sensor_timer >= 100000)
        {
            ...
            // 读取压力传感器（最重要）
            pressure_state = Pressure_GetState();
            ...
            printf("[SENSOR] Press:%d, Dist:%dcm, Radar:%d, PIR:%d, Light:%d%%, T:%dC, H:%d%%\r\n", 
                   pressure_state, distance, radar_state, pir_state, light_percent, temperature, humidity);
        }
...
void System_Init(void)
{
    ...
    printf("\r\n");
    printf("========================================\r\n");
    printf("  Seat Detection System v2.0\r\n");
    ...
    printf("  === Sensors ===\r\n");
    printf("  Pressure      : PA8 (Digital Input)\r\n");
    printf("  Ultrasonic    : PB0(Trig), PB1(Echo)\r\n");
    printf("  Radar         : PA7 (OUT)\r\n");
    printf("  PIR           : PA3 (GPIO Input)\r\n");
...
{
    OLED_Clear();
    OLED_ShowString(1, 1, "Seat Detect v2");
    OLED_ShowString(2, 1, "Multi-Sensor");
...
/*=============================================================================
 * 函数：Update_Seat_State_MultiSensor
 * 功能：多传感器融合判断座位状态
 * 算法：
 *   - 超声波距离 < 50cm → 权重50分
 *   - 雷达检测到人体 → 权重30分
...
void Update_Seat_State_MultiSensor(void)
{
    ...
    {
        occupied_score += 50;
        printf("[FUSION] Ultrasonic: OCCUPIED (+50)\r\n");
    }
    ...
    {
        occupied_score += 30;
        printf("[FUSION] Radar: OCCUPIED (+30)\r\n");
    }
    ...
    {
        occupied_score += 15;
        printf("[FUSION] PIR: MOTION (+15)\r\n");
    }
    
    // 4. 光照传感器判断（权重5%）
    static uint8_t last_light = 100;
    if(light_percent < last_light - 20)
    {
        occupied_score += 5;
        printf("[FUSION] Light: DROPPED (+5)\r\n");
    }
    last_light = light_percent;
    
    // 5. 综合判断
    printf("[FUSION] Total Score: %d/100\r\n", occupied_score);
    
    uint8_t new_state = (occupied_score >= 50) ? SEAT_OCCUPIED : SEAT_VACANT;
...
void Send_Data_To_ESP32(void)
{
    ...
    printf("[ESP32] Data sent - State:%s, Dist:%dcm, T:%dC, H:%d%%, L:%d%%\r\n",
           current_seat_state == SEAT_OCCUPIED ? "OCCUPIED" : "VACANT",
           distance, temperature, humidity, light_percent);
}
...
void Handle_Key_Input(void)
{
    ...
    if(keyNum == 1)  // K1：显示传感器详细信息
    {
        ...
        OLED_ShowNum(1, 6, distance, 3);
        OLED_ShowString(1, 9, "cm");
        
        OLED_ShowString(2, 1, "Radar:");
        OLED_ShowNum(2, 7, radar_state, 1);
        OLED_ShowString(2, 9, "PIR:");
        OLED_ShowNum(2, 13, pir_state, 1);
