...
/*=============================================================================
 * 座位占用检测系统 v3.0 - STM32主程序
 * 功能：多传感器融合检测座位是否有人，并通过WiFi上报状态
 * 硬件：STM32 + ESP32 + 超声波 + 雷达 + 压力传感器 + PIR + DHT11 + 光敏
 * 
 * 传感器配置：
 * - 压力传感器（主要）：PB4(OUT1), PB5(OUT2) - 权重60%
 * - 超声波测距（辅助）：PB0(Trig), PB1(Echo) - 权重25%
 * - 雷达传感器（辅助）：PA7(OUT) - 权重10%
 * - PIR红外（辅助）：PA3(OUT) - 权重5%
 *============================================================================*/
...
void Update_Seat_State_Advanced(void);
...
int main(void)
{
    ...
    printf("[SYSTEM] Seat detection system v3.0 started!\r\n");
    printf("[SYSTEM] Multi-sensor fusion with pressure sensor\r\n\r\n");
    ...
    while (1)
    {
        ...
        if(++sensor_timer >= 100000)
        {
            ...
            printf("[SENSOR] P1:%d P2:%d Dist:%dcm Radar:%d PIR:%d Light:%d%% T:%dC H:%d%%\r\n", 
                   pressure1, pressure2, distance, radar_state, pir_state, 
...
void System_Init(void)
{
    ...
    printf("\r\n");
    printf("========================================\r\n");
    printf("  Seat Detection System v3.0\r\n");
    printf("  Advanced Multi-Sensor Fusion\r\n");
    printf("========================================\r\n");
    ...
    printf("[INIT] Initializing Pressure sensor...\r\n");
    Pressure_Init();
    
    printf("[INIT] Initializing Ultrasonic...\r\n");
    Ultrasonic_Init();
    
    printf("[INIT] Initializing Radar...\r\n");
    Radar_Init();
    ...
    printf("  Ultrasonic    : PB0(Trig), PB1(Echo)\r\n");
    printf("  Radar         : PA7 (OUT)\r\n");
    printf("  PIR           : PA3 (OUT)\r\n");
...
{
    OLED_Clear();
    OLED_ShowString(1, 1, "Seat Detect v3");
    OLED_ShowString(2, 1, "Pressure+Multi");
...
/*=============================================================================
 * 函数：Update_Seat_State_Advanced
 * 功能：高级多传感器融合判断座位状态
 * 算法：
 *   - 压力传感器（任一通道） → 权重60分（主要依据）
 *   - 超声波距离 < 50cm → 权重25分
 *   - 雷达检测到人体 → 权重10分
 *   - PIR检测到运动 → 权重5分
 *   - 总分 >= 60分 → 判定为占用
...
void Update_Seat_State_Advanced(void)
{
    ...
    {
        occupied_score += 60;
        printf("[FUSION] Pressure: OCCUPIED (+60) [CH1:%d CH2:%d]\r\n", pressure1, pressure2);
    }
    ...
    {
        occupied_score += 10;
        printf("[FUSION] Radar: OCCUPIED (+10)\r\n");
    }
    ...
    // 5. 综合判断
    printf("[FUSION] Total Score: %d/100 (Threshold: 60)\r\n", occupied_score);
    
    uint8_t new_state = (occupied_score >= 60) ? SEAT_OCCUPIED : SEAT_VACANT;
...
void Handle_Key_Input(void)
{
    ...
    if(keyNum == 1)  // K1：显示传感器详细信息
    {
        ...
        OLED_ShowNum(3, 3, radar_state, 1);
        OLED_ShowString(3, 5, "PIR:");
        OLED_ShowNum(3, 9, pir_state, 1);
        OLED_ShowString(3, 11, "L:");
        OLED_ShowNum(3, 13, light_percent, 2);
    ...
    else if(keyNum == 4)  // K4：刷新所有传感器
    {
        ...
        printf("[SENSOR] P1:%d P2:%d Dist:%dcm Radar:%d PIR:%d Light:%d%%\r\n",
               pressure1, pressure2, distance, radar_state, pir_state, light_percent);
    }
}
